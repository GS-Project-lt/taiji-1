...
export function wxUserInfo() {
  if (window.location.search && window.location.search.indexOf('code=') > 0){
    ...
    // 链接没有code，跳转微信授权
    window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx1e1c4e1ad79481d6&redirect_uri=${encodeURIComponent(window.location.origin + window.location.pathname + window.location.hash)}&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect`
  }
